...
function mapBookToSchema() {
  let res = [];
  for (const book of books) {
    res.push({
      name: book.title,
      slug: book.title.toLowerCase().replace(/\s+/g, "-"), // Generating slug from the title
      ...
      pages: book.pageCount || 0, // Default to 0 if not provided
      type: "Unknown", // You might want to update this based on your needs
      size: "N/A", // No data provided for size
      price: "N/A",
      rate: 0, // No price in the dataset
      description:
        book.shortDescription ||
        book.longDescription ||
        "No description available.",
      authorName: book.authors ? book.authors[0] : "Unknown", // Take the first author, if available
      categoryName: book?.categories[0], // Take the first category, if available
  ...
  try {
    if (data) {
      ...
      for (const book of data) {
        ...
        let category = await PRISMA.category
          .findUnique({
        ...
        if (!author) {
          author = await PRISMA.author.create({
            ...
            data: {
              name: authorName,
              slug: authorName.toLowerCase().replace(/\s+/g, "-"), // Generating slug from the title

              image: "default-image-url", // or use a proper image URL
              country: "Unknown",
        ...
        const res = await PRISMA.book
          .createMany({
            data: {
              ...
              price,
              description,
              authorName: author?.name || authorName, // Associate the book with the author
              categoryName: category?.name, // Assuming category already exists
            },
            skipDuplicates: true,
          })
          .then((res) => res)
